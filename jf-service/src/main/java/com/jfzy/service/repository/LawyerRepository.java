...
public interface LawyerRepository extends JpaRepository<LawyerPo, Integer> {
	...
	@Transactional
	@Modifying
	@Query("UPDATE LawyerPo SET scored_tasks=scored_tasks+1, score = (score*scored_tasks+?1)/(scored_tasks+1) WHERE id=?2")
	void updateLawyerScore(int score, int lawyerId);
