...
public class WechatServiceImpl {
	...
	public WechatTokenInfo getTokenByCode(String code) throws IOException {
	...
	public UserAccountBo getUserAccountByCode(String code, int userId)  {
		UserAccountBo userAccountBo = null;
		WechatTokenInfo wechatUserInfo = getTokenByCode(code);
		
		// WechatUserInfo wechatUserInfo = new WechatUserInfo();
		// wechatUserInfo.setOpenid("WEB4z2Qwu6xUjDJgDeKar7zcR");
		if (wechatUserInfo != null) {
			...
			if (userAccountBo == null) {
				...
				userService.register(userAccountBo, userId);
				...
				userService.createOrUpdateUser(userBo);
				
				
				
				
				//customerTemp.setUnsubscribed(true);

			}else{
				String global_acess_token = wechat.fetchAccessToken();
				StringBuilder sb = new StringBuilder("https://api.weixin.qq.com/cgi-bin/user/info");
				sb.append("?access_token=").append(global_acess_token).append("&openid=")
						.append(wechatUserInfo.getOpenid()).append("&lang=zh_CN");
				String result = HttpClientUtils.get(sb.toString());
				JsonNode node = JsonUtils.fromJson(result, JsonNode.class);
				if (node.has("errcode")) {
				...
				WechatUser wechatUser = new WechatUser(result);
				customer.setUnsubscribed(wechatUser.getSubscribe() == 0 ? true : false);
